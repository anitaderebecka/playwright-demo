...
import { AuthModal, type Credentials as UserCredentials } from '../../components/authModal'
...
const assertUserLoggedIn = async (authModalComponent: AuthModal, currentUser: CurrentUser): Promise<void> => {
...
const assertErrorDisplayed = async (authModalComponent: AuthModal, expectedError: string): Promise<void> => {
...
test('failed login with invalid email address', { tag: ['@auth', '@critical'] }, async ({ authModal }) => {
...
test('failed login with invalid password address', { tag: ['@auth', '@critical'] }, async ({ authModal }) => {
  const credentials = { ...validCredentials }
...
test('success login after incorrect credentials update', async ({ authModal, currentUser }) => {
  const credentials = { ...validCredentials }
  credentials.password = faker.internet.password()
  await authModal.login(credentials)
  await expect(authModal.errorText).toHaveText(errorMessage)
